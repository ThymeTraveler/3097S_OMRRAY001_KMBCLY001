...
with open('FTP/encrypted.txt', 'rb') as f:
    contents = f.read()
    d.decrypt_message(contents)

import numpy as np
FFTPercent=26
f = open("decrypted.txt", "r") #open file for reading
...
No_items=int(m.ceil(len(Arrayoftest)*(100/FFTPercent))) #calculates the number of items that should be in the orignal uncompressed file
