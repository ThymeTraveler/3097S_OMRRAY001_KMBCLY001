...
def encryptFile(filename, password,outputName):
    ...
    try:
        ...
        hkey = hashObj.digest()
        encryptPath = Path(filePath.parent.resolve().as_posix() + "/" + outputName + ".enc")
...
def decryptFile(filename, password):
    ...
    try:
        ...
        hashObj = SHA256.new(password.encode('utf-8'))
        hkey = hashObj.digest()
        DFP = Path(filePath.parent.resolve().as_posix() + "/" + "output")
